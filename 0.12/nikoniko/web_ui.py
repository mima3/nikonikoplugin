...
class NikoNikoComponent(Component):
    ...
    def get_navigation_items(self, req):
        if req.perm.has_permission('NIKONIKO_VIEW'):
          yield ('mainnav', 'nikoniko', tag.a('ニコカレ', href=req.href.nikoniko()))
    ...
    def process_request(self, req):
        ...
        do_update = False        
        if req.args.has_key('worstMood'):
            do_update = True
            todays_mood = 'worstMood'
        ...
        if req.args.has_key('bestMood'):
            do_update = True
            todays_mood = 'bestMood'
        ...
        # Calendar
        data = self.build_calendar(req, cursor)
            
        # Make variables available to template
        data['username'] = username
        data['comment'] = comment
        data['todays_mood'] = todays_mood
        data['date_time'] = date_time
        data['can_change'] = req.perm.has_permission('NIKONIKO_CHANGE')

        add_stylesheet(req, 'nn/css/nikoniko.css')
        return ('nikoniko.html', data, None)
    ...
    def build_calendar(self, req, cursor):
        ...
        while week_date <= end_of_week:
            days.append(self.getWeekStr(week_date))
        ...
        for row in niko_data:
            ...
            mood_data[username][self.getWeekStr(mood_date)] = mood
            comment_data[username][self.getWeekStr(mood_date)] = comment
            
        # Make variables available to template
        #self.log.info("days %s %s", locale.getlocale(),days[0])
        data = {}
        data['mood_data'] = mood_data
        data['comment_data'] = comment_data
        data['last_week'] = last_week.strftime("%d/%m/%y")
        data['next_week'] = next_week.strftime("%d/%m/%y")
        data['start_of_week'] = start_of_week.strftime("%d/%m/%y")
        data['end_of_week'] =  end_of_week.strftime("%d/%m/%y")
        data['days'] = days 
        data['users'] = users
        return data
